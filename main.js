...
let about //Variável de apoio para resolver o problema de abrir múltiplas instâncias
...
//Encerrar aplicativo ao fechar todas as Janelas do Aplicativo (Windows e Linux)
...
ipcMain.on('new-task', async (event, args) => {
    ...
    if (args.nome === "") {
    ...
    } else if (args.descricao === "") {
        dialog.showMessageBox(win, {
            type: "info",
            message: 'Preencha o nome da Descrição',
            buttons: ['OK']
        })
    } else {
        ...
        //*Observação: Desnecessário no Projeto
        /*dialog.showMessageBox(win, {
            type: 'info',
...
ipcMain.on('update-task', async (event, args) => {
    ...
    if (args.nome === "") {
        ...
        const tarefaEditada = await Tarefa.findByIdAndUpdate(
            args.idTarefa, {
            nome: args.nome,
            descricao: args.descricao //Mudar para o fone*
        },
            {
                new: true
            }
...
ipcMain.on('delete-task', async (event, args) => {
    ...
    const { response } = await dialog.showMessageBox(win, {
        type: 'warning',
        buttons: ['Cancelar', 'Excluir'],   //[0 == Cancelar e 1 == Excluir]
